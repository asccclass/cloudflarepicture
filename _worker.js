...
async function handleRootRequest(request, USERNAME, PASSWORD, enableAuth) {
  ...
  <meta name="description" content="Telegraph圖床-基於Workers的圖床服務">
  <meta name="keywords" content="Telegraph圖床,Workers圖床, Cloudflare, Workers,telegra.ph, 圖床">
  <title>Telegraph圖床-基於Workers的圖床服務</title>
...
<body>
  ...
  <div class="card">
      <div class="title">Telegraph圖床</div>
      <button type="button" class="btn" id="viewCacheBtn" title="檢視歷史記錄"><i class="fas fa-clock"></i></button>
      ...
      <p class="project-link">專案開源於 GitHub - <a href="https://github.com/0-RTT/telegraph" target="_blank" rel="noopener noreferrer">0-RTT/telegraph</a></p>
      ...
      $(document).ready(function() {
        ...
        const tooltipText = enableCompression ? '關閉壓縮' : '開啟壓縮';
        ...
        function copyToClipboardWithToastr(text) {
          ...
          toastr.success('已複製到剪貼簿', '', { timeOut: 300 });
        }
        ...
        $('#viewCacheBtn').on('click', function() {
          ...
          if (isCacheVisible) {
            ...
            if (cacheData.length > 0) {
              ...
              cacheContent.append('<div>還沒有記錄喔！</div>').show();
            }
            isCacheVisible = true;
          }
        });
...
async function generateAdminPage(DATABASE) {
  ...
  <head>
    ...
    async function deleteSelectedImages() {
      ...
      const confirmation = confirm('你確定要刪除選取的媒體檔案嗎？此操作無法撤回。');
  ...
  <body>
    <div class="header">
      <div class="header-left">
        <span>媒體檔案 ${mediaData.length} 個</span>
        <span>已選取: <span id="selected-count">0</span>個</span>
      </div>
      <div class="header-right hidden">
        ...
        <button id="select-all-button" class="delete-button" onclick="selectAllImages()">全選</button>
        <button id="delete-button" class="delete-button" onclick="deleteSelectedImages()">刪除</button>
      </div>
    </div>
    <div class="gallery">
      ${mediaHtml}
...
async function handleUploadRequest(request, DATABASE, enableAuth, USERNAME, PASSWORD, domain, TG_BOT_TOKEN, TG_CHAT_ID, maxSize) {
  try {
    ...
    if (file.size > maxSize) {
      return new Response(JSON.stringify({ error: `檔案大小超過${maxSize / (1024 * 1024)}MB限制` }), { status: 413, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    if (!telegramResponse.ok) {
      ...
      throw new Error(errorData.description || '上傳到 Telegram 失敗');
    }
    ...
    else throw new Error('返回的資料中沒有檔案 ID');
...
async function handleImageRequest(request, DATABASE, TG_BOT_TOKEN) {
  ...
  while (attempts < maxAttempts) {
    ...
    if (!getFilePath.ok) {
      return new Response('getFile請求失敗', { status: 500 });
    }
  ...
  if (!response.ok) {
    return new Response('獲取檔案內容失敗', { status: 500 });
  }
...
async function handleBingImagesRequest(request) {
  ...
  if (!res.ok) {
    return new Response('請求 Bing API 失敗', { status: res.status });
  }
...
async function handleDeleteImagesRequest(request, DATABASE, USERNAME, PASSWORD) {
  ...
  try {
    ...
    if (!Array.isArray(keysToDelete) || keysToDelete.length === 0) {
      return new Response(JSON.stringify({ message: '沒有要刪除的項目' }), { status: 400 });
    }
    const placeholders = keysToDelete.map(() => '?').join(',');
    const result = await DATABASE.prepare(`DELETE FROM media WHERE url IN (${placeholders})`).bind(...keysToDelete).run();
    if (result.changes === 0) {
      return new Response(JSON.stringify({ message: '未找到要刪除的項目' }), { status: 404 });
    }
    const cache = caches.default;
    ...
    return new Response(JSON.stringify({ error: '刪除失敗', details: error.message }), { status: 500 });
  }
}